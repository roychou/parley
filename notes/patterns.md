...
**Decouple decision cadence from mark-to-market cadence.** Strategies (re)decide only on `decision_dates` (expensive, LLM-driven); mark-to-market — stop-loss + equity snapshot — runs every trading day (cheap, and risk must be checked tightly). Coupling them either starves risk control (weekly stop-loss lets a position blow past −20% intra-week) or pays 5× to regenerate near-identical decisions. Consequence: the equity curve is sampled daily, so Sharpe annualizes with 252, not 52 — `periods_per_year` must track the *equity-curve* sampling rate, not the decision rate. When the price data contains only the decision dates the two cadences coincide and behaviour is unchanged (keeps old tests valid).

**Cache at the signal level, not the Decision level; key on data-version, not the calendar date.** Caching the whole Decision means any prompt change invalidates everything and a synthesis change serves stale Decisions. Caching each specialist signal separately gives selective per-specialist invalidation, always-fresh synthesis (re-synthesized from cached signals — it's cheap and deterministic), and a clean seam for a future specialist (e.g. sentiment). The key is the *data version* the signal was computed from, not the decision date: technicals change daily (key on as_of), fundamentals change ~quarterly — so they should be reused across decision dates.

**Separate cache-key fidelity from signal fidelity when an input has a fast-moving component.** The fundamentals signal looked daily-varying because `pe_ratio` is priced at the as-of close — so naively it can't be reused across dates. But the specialist only *acts* on P/E via thresholds ("above 40 high, below 15 low"), so the traded output is stable within a band. Resolution: keep feeding the specialist the exact P/E (so live and backtest stay faithful and the evals are unaffected — no recalibration), but key the cache on `(filing_date, pe_band)`. Fundamentals then recompute only on a new filing or a band crossing. Documented trade-off: a reused signal's *reasoning text* may cite a P/E from another day in the same band (audit imprecision); the traded direction/confidence is band-stable, so trading behaviour is faithful. Make bands finer if exact audit P/E ever matters. The deeper fix — splitting the price-derived P/E out of the cached fundamentals block entirely — is deferred; band-keying gets the cost win without a schema/eval change.
