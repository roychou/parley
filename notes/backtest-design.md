...
5. **Single-factor baselines use the same universe.** P/E ranking and RSI baselines pick from the same 15 tickers. A more rigorous baseline would let them pick from the full S&P 500. Acknowledged scope limit.
...
5. **Price-history depth at the loader seam (Day 43 blocker).** The `run.py` entrypoint wires `get_prices` as the `price_loader`, but `fetch_raw_history` defaults to `period="1y"`. That covers the small-window validation (recent dates), but the full 6-month run needs depth = window + indicator lookback. The earliest decision dates would otherwise have thin or empty trailing history — `get_fundamentals_as_of` returns `None` when no price exists on-or-before the as-of date, and technicals snapshots silently shrink (observed: a `2025-09-15` snapshot started at `2025-05-28`, ~3.5 months). The fix is at the seam, not in the data layer: pass a 5y price loader (e.g. `partial`/wrapper calling `get_prices` with `period="5y"`) into `run_backtest` before the Day 43 run. Free tier provides 5 years, so this is a parameter, not a constraint.
