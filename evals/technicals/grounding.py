...
class TechnicalClaimVerdict(BaseModel):
    ...
    explanation: str = Field(
        description=(
            "Why it is grounded/ungrounded. If ungrounded, explicitly state if the "
            "data was wrong, the interpretation was flawed, or the required metrics "
            "were missing."
        )
    )
...
class TechnicalsGroundingEval:
    ...
    async def run(self, input_data: TechnicalsAnalysis) -> EvalResult:
        ...
        system_prompt = """
You are evaluating whether a quantitative analyst's technical reasoning is faithfully grounded \
in the supporting data they were given.

You will receive:
1. SUPPORTING_TECHNICALS: a JSON object of technical indicators and time horizons. Treat this \
as ground truth.
2. REASONING: a paragraph of technical analysis written by the agent.

Your job: identify every technical claim in REASONING. Decompose each claim into one of three \
categories and evaluate it.

Categories & Grounding Rules:
1. NUMERIC_INTERPRETATION (e.g., "RSI of 72 suggests overbought")
- GROUNDED if: The exact number exists in SUPPORTING_TECHNICALS, AND the interpretation aligns \
with universally accepted technical analysis standard heuristics (e.g., RSI > 70 is overbought, \
< 30 is oversold).
- UNGROUNDED if: The number is wrong, or the interpretation is objectively false (e.g., calling \
RSI 20 "overbought").

2. PATTERN (e.g., "Death cross formed", "Price broke above SMA")
- GROUNDED if: The specific indicators required to form the pattern are explicitly present in \
the data, AND their mathematical relationship supports the pattern.
- UNGROUNDED if: The required indicators are missing entirely (e.g., claiming a "Death Cross" \
when only SMA-20 is provided in the data), or the math contradicts the pattern.

3. DIRECTIONAL / TEMPORAL (e.g., "Momentum is building", "Trend reversed")
- GROUNDED if: The data contains sufficient temporal history (e.g., an explicit date range \
spanning weeks/months) to prove the change over time.
- UNGROUNDED if: The data is only a single day's snapshot or lacks the historical context \
needed to prove a "trend".

Be strict. If the agent claims a pattern or trend but the provided data does not contain the \
specific fields to prove it, mark it UNGROUNDED and explain that the data was missing.
